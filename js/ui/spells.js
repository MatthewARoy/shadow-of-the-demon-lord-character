...
import { esc, rulesTables } from "./util.js";

// Lowercased text of a spell's captured tables, memoised — the search runs
// this over the whole corpus on every keystroke.
const tableTextCache = new WeakMap();
function tableText(s) {
  let t = tableTextCache.get(s);
  if (t === undefined) {
    t = (s.tables || []).flatMap((tb) => tb.rows.flat()).join(" ").toLowerCase();
...
function spellModalHtml(s) {
  ...
  <div class="spell-modal-box" role="dialog" aria-modal="true" aria-label="${esc(s.name)} details" tabindex="-1">
    ...
    ${scoreSec}
    ${lensSec}
    <div class="sm-section"><h4>Description</h4><p class="sm-desc">${esc(s.description)}</p>${rulesTables(s.tables)}</div>
    <div class="sm-foot">
