...
CONTROL_CONDS = [
    "stunned", "immobilized", "slowed", "prone", "grabbed", "restrained",
    "paralyzed", "incapacitated", "dazed", "blinded", "deafened", "asleep",
    "surprised", "defenseless",
]
...
NEGATE = re.compile(
    ...
    r"instead of (?:being |becoming )?)", re.I)


# A condition whose subject is the caster ("you become stunned") is a self-risk
# drawback, not control/fear inflicted on a target.
SELF_SUBJECT = re.compile(r"\byou (?:become|are|'re|gain|suffer|turn|fall|also become|might (?:become|be)|may (?:become|be)|could become|would become|instead become)\b[^.]{0,18}$", re.I)
...
RULES += [
    ...
    ("auto-damage", "offense", lambda d, s: bool(_DAMAGE.search(d)) and "attack" not in s),
]
...
RULES += [
    ...
    ("buff-attack", "support", rx(r"\bboons?\b[^.]{0,70}attack roll|attack rolls\b[^.]{0,40}\bwith \d+ boons?\b")),
    ("buff-challenge", "support", rx(r"\bboons?\b[^.]{0,70}challenge roll|challenge rolls\b[^.]{0,40}\bwith \d+ boons?\b")),
    ("reroll", "support", rx(r"rerolls?|roll(?:s)? (?:it |that |the (?:die|roll) )?twice|roll an additional die and use the (?:high|great)|use the (?:higher|highest|better|best) (?:of the )?result")),
]

# ---- CONTROL / DEBUFF ----------------------------------------------------
RULES += [
    ("debuff-rolls", "control", rx(r"\bbanes?\b[^.]{0,70}(?:attack|challenge|roll)|(?:attack|challenge) rolls\b[^.]{0,40}\bwith \d+ banes?\b")),
    ("control", "control", lambda d, s: cond_applied(d, CONTROL_CONDS)),
    ("fear", "control", lambda d, s: cond_applied(d, ["frightened", "horrified"]) or bool(re.search(r"\bflees?\b|must flee", d))),
    ...
    ("mind-control", "control", lambda d, s: cond_applied(d, ["charmed"]) or bool(re.search(r"becomes? compelled|is compelled|compelled to (?:obey|serve|attack|move|act|fight|do|use|make)|\bdominat\w+|takes? control of (?:the|its|each|your)|control (?:the|its|each|your) (?:target|creature|action|mind|body|movement)", d))),
    # Inflicts Insanity on a target (self-Insanity is a cost, handled below).
    ("insanity", "control", rx(r"(?:target|creature|it|each \w+|they|them) (?:gains?|takes?|suffers?|marks?)\b[^.]{0,30}Insanity|spreads? (?:madness|Insanity)")),
]

# ---- SUPPORT / DEFENSE ---------------------------------------------------
# Restores Health on cast.
_HEAL = re.compile(r"heals? damage equal to|heals? \d+(?:d\d+)? damage|heals? all (?:its |your |the )?damage|regains?\b[^.]{0,30}\bHealth\b|recover[^.]{0,30}\bHealth\b", re.I)
...
_HEAL_SUP = re.compile(r"heals? (?:\d+d\d+ |\w+ )?extra damage|each time[^.]{0,30}heals[^.]{0,25}(?:damage|extra)|whenever[^.]{0,40}heals (?:damage|\d)|consume[^.]{0,40}\bheal\w*\b[^.]{0,20}damage", re.I)
RULES += [
    ("heal", "support", lambda d, s: bool(_HEAL.search(d))),
    ("heal-support", "support", lambda d, s: bool(_HEAL_SUP.search(d)) and not _HEAL.search(d)),
    ...
    ("protection", "support", rx(r"immune to[^.]{0,40}damage|\bimmunity\b|tak(?:es?|ing) (?:half|no) damage from (?!the attack|this attack|the spell|that attack)|reduce[sd]? (?:the )?damage (?:you|it|the target|they) takes?|damage (?:you|it|the target|they) takes? is (?:reduced|halved)|cannot be (?:targeted|harmed)|invulnerab")),
]

# ---- MOBILITY ------------------------------------------------------------
RULES += [
    ("teleport", "mobility", rx(r"teleports?|instantly (?:move|appear|transport|travel)|vanish\w* and (?:reappear|appear)")),
    ...
    ("fly", "mobility", rx(r"can fly\b|gains? a (?:fly|flying) Speed|\bfly(?:ing)? Speed\b|fly up to|fly (?:over|through|across|toward|away|into)|move by flying|(?:you|it|the target|they) (?:can )?flies?\b|levitat\w+|hover\w* (?:in (?:the )?air|above|off the ground|motionless)|(?:lift|rise|float|soar)\w*[^.]{0,18}(?:in|into) the air|takes? to the air|move(?:s)? (?:up |down )?through the air")),
    ("movement", "mobility", rx(r"bonus to Speed|increase[^.]{0,18}Speed|Speed (?:score )?(?:becomes|increases?)|ignore[^.]{0,20}difficult terrain|move across (?:liquid|the surface|open)|climb\w* Speed|walk (?:on|up|across)|move up to twice|without (?:triggering|provoking) (?:a |any )?free attack")),
]

# ---- UTILITY -------------------------------------------------------------
RULES += [
    ("summon", "utility", rx(r"\bsummons?\b|\bconjures?\b|\b(?:appears?|materializes?)\b[^.]{0,40}(?:within|in an? |on a solid)|compelled[^.]{0,20}(?:monster|creature)|creates? an? [^.]{0,40}(?:that|which) (?:obey|serve|attack|fight|act)")),
    ...
    ("concealment", "utility", rx(r"(?:become[s]?|turn[s]?|render[s]?|are|is|gains? )\w* ?invisib\w+|invisible to|invisibility|becomes? (?:obscured|hidden|indistinct)|cannot be seen|hidden from|conceal\w+ (?:you|it|the target|yourself|itself)|heavily obscured")),
    ...
    ("divination", "utility", rx(r"\bdetects?\b|sense the (?:presence|location|direction|number)|\bscry\w*|learn (?:the|about|whether|if)|read[^.]{0,15}(?:mind|thoughts|memor)|know (?:the (?:exact|location|number|name|direction)|whether|if)|locate the|pinpoint|see (?:the )?(?:aura|invisible)|see through (?:walls|illusions|the)|see into (?:the )?(?:mind|future|past|veil)|detect the presence|discern the")),
    ("transform", "utility", rx(r"transform\w*|polymorph\w*|becomes? an? [^.]{0,30}(?:size \d+|creature|monster|animal|beast)|change (?:its|your|the target.?s)[^.]{0,15}(?:shape|form)|assume the (?:form|shape) of")),
]

# ---- COST / RISK ---------------------------------------------------------
RULES += [
    ("sacrifice", "cost", lambda d, s: "sacrifice" in d),
    ("corruption", "cost", rx(r"you (?:gain|mark|take)\b[^.]{0,18}Corruption|gain \d+ Corruption")),
    ("self-risk", "cost", rx(r"you (?:take|gain|suffer|mark)\b[^.]{0,18}(?:\d+d\d+ damage|\d+ damage|Insanity|Corruption)|you must get a success|or you (?:become|take|gain|suffer|are)\b|you (?:become|are) (?:stunned|impaired|fatigued|injured|cursed)")),
]
...
def main():
    ...
    print(f"tagged {len(spells)} spells ({applied_overrides} via overrides) -> {SPELLS}", file=sys.stderr)
